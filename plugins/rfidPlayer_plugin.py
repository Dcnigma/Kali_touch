...
class RfidPlayerPlugin(QWidget):
    def __init__(self, parent=None, apps=None, cfg=None):
        ...
        if LIB_AVAILABLE:
            ...
            self.log_message("MFRC522 library not available!")
    ...
    def update_grid(self):
        ...
        page_items = list(self.video_map.items())[self.page * VIDEOS_PER_PAGE:(self.page + 1) * VIDEOS_PER_PAGE]
        for i, (uid, video) in enumerate(page_items):
            ...
            self.grid_layout.addWidget(uid_label, row, col)
            self.grid_layout.addWidget(uid_input, row, col + 1)
            self.grid_layout.addWidget(video_input, row, col + 2)
            self.uid_inputs.append(uid_input)
            self.video_inputs.append(video_input)

    # ---------------------- Pagination ----------------------
    def next_page(self):
        total_pages = (len(self.video_map) + VIDEOS_PER_PAGE - 1) // VIDEOS_PER_PAGE
    ...
    def check_card(self):
        ...
        if status == self.reader.MI_OK:
            ...
            if status == self.reader.MI_OK:
                ...
                self.handle_video_for_uid(uid_str)
    ...
    def stop_current_video(self):
        try:
            if self.my_subprocess:
                self.my_subprocess.kill()
        except Exception:
            pass
        time.sleep(0.2)
        self.my_subprocess = None

    def handle_video_for_uid(self, uid_str):
        if uid_str in self.video_map:
            ...
            if os.path.exists(video_path):
                cmd = ["/bin/ffplay", "-fs", "-loop", "0", "-autoexit", video_path]
