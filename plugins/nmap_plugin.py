...
class NmapPlugin(QWidget):
    """
    Touchscreen-friendly Nmap plugin that will automatically prompt to use pkexec when necessary.
    Use as: "plugins.nmap_plugin:NmapPlugin" in apps.json
    """
    name = "Nmap Scanner"
    ...
    def __init__(self, parent=None):
        ...
        # EXPORT button (properly inside __init__)
    ...
    def start_qprocess(self, program: str, args: List[str]):
        ...
        program: executable (e.g. "nmap" or "pkexec")
        args: list of arguments (for pkexec, first argument should be "nmap")
        """
        if self.process:
        ...
        if privileged_needed and not privileged_available:
            ...
            if shutil.which("pkexec"):
                resp = QMessageBox.information(
                    self,
                    "Privileges required",
                    "The selected options require raw socket privileges (root or file capabilities).\n\n"
                    "Do you want to run this scan with administrative privileges now (pkexec)?\n\n"
                    "Yes = run as root via pkexec (recommended)\n"
                    "No = run a safer TCP Connect scan (-sT) instead",
    ...
    def on_stderr(self):
        ...
        if ("dnet: Failed to open device" in raw or "Failed to open device" in raw or "libdnet" in raw) \
                and not self._has_retried and self.allow_auto_fallback:
    ...
    def on_finished(self):
        ...
        self.process = None
        self.export_btn.setEnabled(True)
        # Auto-save result (optional): comment out if you don't want auto-save
        try:
